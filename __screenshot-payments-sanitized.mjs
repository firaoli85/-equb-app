...
await page.screenshot({ path: path.join(OUT, "payments-sanitized--light.png"), fullPage: false });
console.log("✓ payments light screenshot saved");

await db.memberSession.delete({ where: { sessionToken: session.sessionToken } }).catch(() => {});
await browser.close();
await db.$disconnect();
console.log("✅ Done → ui-motion/payments-sanitized--light.png");
