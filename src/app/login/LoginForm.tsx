  ...
  function submitPin(pinValue: string) {
    startVerify(async () => {
      ...
      if (result?.noPin) {
        ...
        setLocked(true);
        setLockedMinutes(result.lockedMinutes ?? 30);
        setPin("");
        ...
        <div className="space-y-5">
          ...
          {hasPin === false ? (
            <div className="text-center py-6 space-y-3">
              <div className="w-14 h-14 mx-auto rounded-2xl bg-amber-100 dark:bg-amber-950 flex items-center justify-center">
                <svg className="w-7 h-7 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                </svg>
              </div>
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">
                Your PIN has not been set yet.
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Please contact your Equb manager.
              </p>
            </div>
          ) : locked ? (
            /* Locked */
