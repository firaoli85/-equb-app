...
const OUT = path.join(__dirname, "ui-payments-before");
...
await page.screenshot({ path: path.join(OUT, "member-payments--light--390.png"), fullPage: true });
console.log("✓ light");

// Dark
await page.evaluate(() => document.documentElement.classList.add("dark"));
await page.waitForTimeout(200);
await page.screenshot({ path: path.join(OUT, "member-payments--dark--390.png"), fullPage: true });
console.log("✓ dark");

await db.memberSession.delete({ where: { sessionToken: session.sessionToken } }).catch(() => {});
await browser.close();
await db.$disconnect();
console.log("✅ Saved to ui-payments-before/");
